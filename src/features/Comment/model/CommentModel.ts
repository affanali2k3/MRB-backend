...
export class Comment extends Model {
    ...
    public static ID = "comment_id" as string; // Define the column name for the comment ID
    public static POST_ID = "post_id" as string; // Define the column name for the post ID
    public static USER_EMAIL = "user_email" as string; // Define the column name for the user's email
    public static TEXT = "text" as string; // Define the column name for the comment text
    ...
    @Column({
        ...
        allowNull: false,
        references: { model: 'users', key: 'user_email' }, // Set up a foreign key relationship with the "users" table
    })
    userEmail!: string; // Define the user's email column
