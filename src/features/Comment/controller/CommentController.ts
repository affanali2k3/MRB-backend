...
class CommentController {
    ...
    async saveComment(req: Request, res: Response) {
        try {
            const data: CommentData = req.body; // Extract comment data from the request body
            const commentId: number = await CommentRepository.saveComment({ userEmail: data.userEmail, postId: data.postId, text: data.text }); // Call the CommentRepository method to save the comment
