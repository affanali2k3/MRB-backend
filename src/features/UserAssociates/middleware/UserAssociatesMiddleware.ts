...
class UserAssociatesMiddlware {
  async sendRequestVerify(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
  ...
  async acceptRequestVerify(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      const userId: number = req.body.receiverId;
      const userToken: string | undefined = req.headers.authorization;
...
export default new UserAssociatesMiddlware();
