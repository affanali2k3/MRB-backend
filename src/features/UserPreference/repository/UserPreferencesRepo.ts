...
class UserPreferenceRepo implements IUserPreferenceRepo {
    ...
    async getPreference({userId}: {userId: number}): Promise<UserPreferences> {
        try {
            const userPreference = await UserPreferences.findOne({where: {userId: userId}});
    ...
    async updatePreference(data: UserPreferencesData): Promise<UserPreferences> {
        try{
            const userPreference = await UserPreferences.findOne({where: {userId: data.userId}});
