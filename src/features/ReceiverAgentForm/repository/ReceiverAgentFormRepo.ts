...
import {
  ReceiverAgentFormType,
  ReceiverAgentFormValues,
} from "../controller/ReceiverAgentFormController";
...
interface IReceiverAgentFormRepo {
  ...
  getOpenFormsProposalsReceivedByUser({
    userId,
  }: {
    userId: number;
  }): Promise<ReceiverAgentOpenForm[]>;
  getDirectFormsProposalsReceivedByUser({
    userId,
  }: {
    userId: number;
  }): Promise<ReceiverAgentDirectForm[]>;
  getOpenFormsSent({
    userId,
  }: {
    userId: number;
...
class SenderAgentFormRepo implements IReceiverAgentFormRepo {
  async getOpenFormsProposalsReceivedByUser({
    ...
    try {
      const openFormsProposalsReceived: ReceiverAgentOpenForm[] =
        await ReceiverAgentOpenForm.findAll({
          include: [
            {
  ...
  async getDirectFormsProposalsReceivedByUser({
    ...
    try {
      const directFormsProposalsReceived: ReceiverAgentDirectForm[] =
        await ReceiverAgentDirectForm.findAll({
          include: [
            {
  ...
  async changeOpenFormConsideringStatus({
    receiverAgentOpenFormId,
    status,
  }: {
    receiverAgentOpenFormId: number;
    status: string;
  }): Promise<void> {
    try {
      const openFormsProposalsReceived: ReceiverAgentOpenForm | null =
        await ReceiverAgentOpenForm.findOne({
          where: {
            id: receiverAgentOpenFormId,
          },
        });

      if (!openFormsProposalsReceived)
  ...
  async changeDirectFormConsideringStatus({
    receiverAgentDirectFormId,
    status,
  }: {
    receiverAgentDirectFormId: number;
    status: string;
  }): Promise<void> {
    try {
      const directFormsProposalsReceived: ReceiverAgentDirectForm | null =
        await ReceiverAgentDirectForm.findOne({
          where: {
            id: receiverAgentDirectFormId,
          },
        });

      if (!directFormsProposalsReceived)
        throw new Error("The form does not exist");
  ...
  async getOpenFormsSent({
    userId,
  }: {
    userId: number;
  }): Promise<ReceiverAgentOpenForm[]> {
    try {
      const formsSent: ReceiverAgentOpenForm[] =
        await ReceiverAgentOpenForm.findAll({
          where: {
  ...
  async createForm(values: ReceiverAgentFormValues): Promise<void> {
    try {
      if (values.formType === ReceiverAgentFormType.Direct) {
        const senderAgentForm: SenderAgentDirectForm | null =
          await SenderAgentDirectForm.findOne({
            where: { id: values.senderAgentFormId },
          });

        if (!senderAgentForm)
          throw new Error("Form to apply for does not exist");

        if (values.receiverAgent === senderAgentForm.senderAgent)
          throw new Error("Cannot apply to your own referral");
        ...
        const senderAgentForm: SenderAgentOpenForm | null =
          await SenderAgentOpenForm.findOne({
            where: { id: values.senderAgentFormId },
          });
