...
interface IChatRepo {
    ...
    getAllMessages({ senderId, receiverId }: { senderId: number, receiverId: number }): Promise<Message[]>;
    createChat({ senderId, receiverId, message }: { senderId: number, receiverId: number, message:string }): Promise<void>;
...
class ChatRepo implements IChatRepo {
    ...
    async getAllMessages({ senderId, receiverId }: { senderId: number, receiverId: number }): Promise<Message[]> {
        try {
            ...
            const messages: Message[] = await Message.findAll({
                where: Sequelize.or(
                    ...
                    { senderEmail: receiverId, receiverId: senderId }
                )
            });
            return messages;
        } catch (err) {
    ...
    async createChat({ senderId, receiverId, message }: { senderId: number, receiverId: number, message:string }): Promise<void>{
        try{
    ...
    async getAllChats({ userId}: { userId: number }): Promise<Chat[]> {
        try {
            const chats: Chat[] = await Chat.findAll({where: {userOneId: userId},
                include: [
                    {
                        model: User,
