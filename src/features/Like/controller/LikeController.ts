...
class LikeController {
  ...
  async removeLike(req: Request, res: Response) {
    try {
      ...
      await LikeRepository.removeLike({
        likeId: data.likeId,
        postId: data.postId,
      }); // Call the LikeRepository to remove the like
  ...
  async getPostLikes(req: Request, res: Response) {
    try {
      ...
      const likes: Like[] = await LikeRepository.getPostLikes({
        postId: postId,
      }); // Call the LikeRepository to get likes for the post
