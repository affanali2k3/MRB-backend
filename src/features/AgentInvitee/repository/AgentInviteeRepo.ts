...
interface IAgentInviteeRepo {
    saveInvitee({ inviterEmail, inviteeEmail }: { inviterEmail: string, inviteeEmail: string }): Promise<void>
    getAllInviteesForUser({ userEmail }: { userEmail: string }): Promise<AgentInvitee[]>
    deleteInvitee({ inviteId }: { inviteId: number }): Promise<void>
}

class AgentInviteeRepo implements IAgentInviteeRepo {
    async saveInvitee({ inviterEmail, inviteeEmail }: { inviterEmail: string, inviteeEmail: string }): Promise<void> {
        try {
    ...
    async getAllInviteesForUser({ userEmail }: { userEmail: string }): Promise<AgentInvitee[]> {
        try {
            const agentInvitees: AgentInvitee[] = await AgentInvitee.findAll({ where: { inviterUserEmail: userEmail } });
    ...
    async deleteInvitee({ inviteId }: { inviteId: number }): Promise<void> {
        try {
            const agentInvitee: AgentInvitee | null = await AgentInvitee.findOne({ where: { id: inviteId } });
